...
object NiLexLanguage : Language.HasFileExt.Impl("nlx"), Language.Construction<Json.Array> {
    ...
    private val meta = (NiLexTokenizerFactory()
            ...
            + verbatim(listOf("import", "keep", "verbatim", "streak", "...", "as"))
            )()
    ...
    private fun addToArray(array: Json.Array.Mutable, code: Code) {
        ...
        val q = ArrayDeque((tokens of code)!!.filter {
            it.type !is NiLexTokenizer.Kept.Keeper && it.type.name !in gaps
        })
        while (q.isNotEmpty()) {
    ...
    private fun verbatim(data: Any?) = Json.Object.of(
        "kind" to "verbatim",
        "data" to data,
    )

    private fun streak(name: String?, charSet: Any?) = Json.Object.of(
        "kind" to "streak",
        "name" to name,
        "char-set" to charSet,
    )
